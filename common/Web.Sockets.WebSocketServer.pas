unit Web.Sockets.WebSocketServer;
...
begin
 ...
 LMsg := [ C_CharFlag ];
 if Length(RawData) <= 125 then  LMsg := LMsg + [Length(RawData)]
  else if (Length(RawData) >= 126)
      and (Length(RawData) <= C_Int ) then
  LMsg := LMsg + [126,(Length(RawData) shr 8) and C_Bytes, Length(RawData) and C_Bytes ]
  else
    LMsg := LMsg + [127,
      (int64(Length(RawData)) shr 56) and C_Bytes,
      (int64(Length(RawData)) shr 48) and C_Bytes,
      (int64(Length(RawData)) shr 40) and C_Bytes,
      (int64(Length(RawData)) shr 32) and C_Bytes,
      ( Length(RawData) shr 24 ) and C_Bytes,
      ( Length(RawData) shr 16 ) and C_Bytes,
      ( Length(RawData) shr 8  ) and C_Bytes,
        Length(RawData)          and C_Bytes ];
